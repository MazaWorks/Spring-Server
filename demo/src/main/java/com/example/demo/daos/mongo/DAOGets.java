...
public class DAOGets {
    ...
    public void create(SedeJJOODtos devuelto, String status) {
        Gson gson = new Gson();
        String jsonDevuelto = gson.toJson(devuelto);
        ...
        doc.append("DEVUELTO", JSON.parse(jsonDevuelto));
        doc.append("STATUS", status);

        dbs.getCollection("GET").insertOne(doc);
    }

    private Integer findIdentifier() {
        Integer id = null;
        MongoCursor<Document> iterator = dbs.getCollection("GET").find().sort(orderBy(ascending("_id"))).iterator();
        if (!iterator.hasNext()){
            id = 1;
        } else {
    ...
    public void create(List devuelto, String status) {
        Gson gson = new Gson();
        String jsonDevuelto = gson.toJson(devuelto);
